...
# Compile script for Hebrew Beamer presentation
# Compiles main.tex using XeLaTeX with proper bibliography processing

set -e  # Exit on any error

echo "🔧 Compiling Hebrew Beamer presentation..."
...
rm -f main.aux main.bbl main.bcf main.blg main.fdb_latexmk main.fls main.log main.out main.run.xml main.synctex.gz main.toc main.nav main.snm
rm -f .logs/main.*
...
move_aux_files() {
    ...
    for file in main.aux main.bcf main.blg main.fdb_latexmk main.fls main.log main.out main.run.xml main.synctex.gz main.toc main.nav main.snm; do
        if [ -f "$file" ]; then
            mv "$file" ".logs/"
        fi
    done
...
if [ -f "main.pdf" ]; then
    echo ""
    echo "🎉 Hebrew presentation compiled successfully!"
    ...
    if command -v xdg-open >/dev/null 2>&1; then
        read -p "📖 Open presentation in default viewer? [y/N]: " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            xdg-open main.pdf &
        fi
...
echo "✨ Hebrew presentation compilation completed successfully!" 
